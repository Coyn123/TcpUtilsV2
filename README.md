...
Together these give you a working, dependency-free HTTP **and WebSocket** server today, serving static files out of `templates/` with basic routing, plus WebSocket upgrade + handshake + an echoing frame loop (verified end-to-end against a real client, not just unit-level). The transport core — `Listener`, `Connection`, `IStream`/`Stream`, `BufferedReader` — is feature-complete for basic reads and writes.
...
- Concurrency is proof-of-concept only: `Main.cpp` spawns one thread per connection and joins it immediately, so requests are still handled one at a time. Direction here has shifted from the worker-pool/queue idea above to thread-per-connection with a bounded concurrent cap instead — in progress.
...
```bash
# Linux/macOS
g++ -std=c++20 -Isrc src/Main.cpp src/transport/Listener.cpp src/transport/Connection.cpp src/transport/Stream.cpp src/transport/BufferedReader.cpp src/http/HttpParser.cpp src/tasks/HttpTask.cpp src/tasks/WSTask.cpp src/websocket/WebSocketFrame.cpp src/util/Base64.cpp src/util/Sha1.cpp -o server -pthread

# Windows (MinGW)
g++ -std=c++20 -Isrc src/Main.cpp src/transport/Listener.cpp src/transport/Connection.cpp src/transport/Stream.cpp src/transport/BufferedReader.cpp src/http/HttpParser.cpp src/tasks/HttpTask.cpp src/tasks/WSTask.cpp src/websocket/WebSocketFrame.cpp src/util/Base64.cpp src/util/Sha1.cpp -o server.exe -lws2_32
```
